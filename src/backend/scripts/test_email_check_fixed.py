...
# 3. 直接使用 SQL 查询
direct_sql = db_session.execute(text(f"SELECT * FROM users WHERE email = '{email_to_check}'"))
...
try:
    ...
    # 显示用户详细信息
    if user_by_email_after:
        print(f"用户ID: {user_by_email_after.id}")
        print(f"用户名: {user_by_email_after.username}")
        print(f"邮箱: {user_by_email_after.email}")
        print(f"角色: {user_by_email_after.role}")
        print(f"密码哈希: {user_by_email_after.password_hash}")
