...
async def get_content_detail(id: int, db: Session = Depends(get_db)):
    """获取内容详情"""
...
async def create_content(request: ContentCreateRequest, db: Session = Depends(get_db)):
...
async def update_content(id: int, content: ContentUpdate, db: Session = Depends(get_db)):
    """更新内容"""
...
@router.delete("/{id}")
async def delete_content(id: int, db: Session = Depends(get_db)):
    """删除内容"""
...
async def submit_for_review(id: int, db: Session = Depends(get_db)):
    """提交审核"""
...
async def approve_content(id: int, request: ApproveRequest, db: Session = Depends(get_db)):
    """审核通过"""
...
async def reject_content(id: int, request: RejectRequest, db: Session = Depends(get_db)):
    """审核拒绝"""
