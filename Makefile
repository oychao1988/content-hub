...
up: ## 启动所有服务
	...
	docker-compose up -d
	@echo "$(GREEN)服务已启动！$(NC)"
	@echo "$(BLUE)前端地址: http://localhost:18030$(NC)"
	@echo "$(BLUE)后端 API: http://localhost:18010/docs$(NC)"
...
status: ## 显示服务状态和健康检查
	...
	@echo ""
	@echo "$(GREEN)健康检查:$(NC)"
	@curl -s http://localhost:18030/health && echo "前端: OK" || echo "前端: FAIL"
	@curl -s http://localhost:18010/docs > /dev/null && echo "后端: OK" || echo "后端: FAIL"
